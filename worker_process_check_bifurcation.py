...
class PAS_Manager:
    ...
    def update_layer_acgrad_window(self, layer_id, model):
        ...
        print('- acc value for layer: '+str(layer_id)+': ' +str(list(model.parameters())[layer_id].detach().cpu().numpy().reshape(-1)[0]))
        sys.stdout.flush()
...
#        self.layer_window[layer_id].append(layer_gradient.numpy().reshape(-1)[0])
    ...
    def check_and_sync(self, model, iter_id):
        result = []
        if iter_id % 128 == 0: # if this layer shall be synchronized at the current iteration
...
#            print('--- start batch '+ str(batch_id) + ' ---')
    ...
    #        logging(' iteration ' + str(iter_id) + ' finished')
            ...
            iter_id += 1
            pas_manager.check_and_sync(model, iter_id)
