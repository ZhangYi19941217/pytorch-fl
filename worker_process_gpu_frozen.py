...
class PAS_Manager:
    def __init__(self, model, epoch_per_round, world_size, group):
        ...
        s_index = 0
        flattened_grad = torch.tensor([]).cuda()
        ...
        self.synchronization_mask = torch.ones(self.flattened_shape).byte().cuda()
        self.global_ac_grad = torch.zeros(self.flattened_shape).cuda()
        self.last_global_ac_grad = torch.zeros(self.flattened_shape).cuda()

        self.ema_alpha = 0.99
        self.swap_ema = torch.zeros(self.flattened_shape).float().cuda()
#        self.swap_ema = [0.0]*len(self.flattened_shape)
        self.global_ac_grad_ema = torch.zeros(self.flattened_shape).float().cuda()
        self.global_abs_ac_grad_ema = torch.zeros(self.flattened_shape).float().cuda()
        self.swap_ema_threshold = 0.5
        self.global_ac_grad_ema_threshold = 0.1 # this shall be larger than 1-self.ema_alpha

        self.round_id = 0
        self.frozen_lengths = torch.zeros(self.flattened_shape).int().cuda()
        self.defrozen_round_ids = torch.zeros(self.flattened_shape).int().cuda()
...
#        self.swap_ema = torch.where(self.global_ac_grad * self.last_global_ac_grad < 0, self.swap_ema*self.
#        self.swap_ema = self.swap_ema * self.ema_alpha + (self.global_ac_grad * self.last_global_ac_grad < 0).float() * (1.0 - self.ema_alpha)
    ...
    def sync(self, iter_id):
        if iter_id % self.sync_frequency == 0:
            ...
            if iter_id % self.evaluate_frequency == 0:
                ...
                self.round_id += 1
                self.synchronization_mask = torch.where(self.defrozen_round_ids == self.round_id, torch.tensor(1).cuda().byte(), torch.tensor(0).cuda().byte())
    ...
    def filter(self, iter_id):
        ...
        for g in grad:
            frag = g.view(-1)
            flattened_grad = torch.cat((flattened_grad, frag),0).cuda()

        filtered_grad = torch.where(self.synchronization_mask > 0, flattened_grad, torch.zeros(self.flattened_shape).cuda()).cuda()
